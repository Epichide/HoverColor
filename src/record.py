...
from .wid_utils.flow_wid import FlowLayout
...
class ColorSpaceGroup(QtWidgets.QGroupBox):
    ...
    def __init__(self, parent=None, wid: BaseWidget = None):
        ...
        self.setSizePolicy(
                    QtWidgets.QSizePolicy.Preferred,  # 水平方向
        ...
        self.option_layout.setSpacing(3)
...
class MetricsGroup(QtWidgets.QWidget):
    def set_default_size_policy(self, widgets):
        """
        统一设置多个组件的SizePolicy
        参数:
            widgets: 组件列表，需要设置相同SizePolicy的组件
        """

        # 为每个组件应用设置
        for widget in widgets:
            if isinstance(widget, QtWidgets.QWidget):  # 确保是QWidget子类
                widget.setSizePolicy(
                    QtWidgets.QSizePolicy.Preferred,  # 水平方向
                QtWidgets.QSizePolicy.Minimum)  # 垂直方向)
    def __init__(self, parent=None):
        ...
        self.info_lab = QLabel("Choose Metrics:", self)
        ...
        self.choose_labs_wid=QWidget(self)

        self.choose_layout = FlowLayout(self.choose_labs_wid)
        self.choose_labs_wid.setLayout(self.choose_layout)

        self.device_grids.addWidget(self.choose_labs_wid, 1, 0, 1, 4)
        self.choose_layout.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
        self.device_grids.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)

        for col in range(4):
            self.device_grids.setColumnStretch(col, 1)

        self.device_grids.setRowStretch(3, 1)  # 第1行（空行）拉伸占满剩余空间
        self.set_default_size_policy([self.choose_labs_wid,])

    def init_ui(self, colorspace_wids: List[BaseWidget]):
        self.colorspace_dicts.clear()
        nrow = 3
        self.colorsp_choosess=[]
        for n, colorsp in enumerate(colorspace_wids):
            ...
            nrow = n // 4 + 3
            self.device_grids.addWidget(colorsp_chooses, n // 4 + 3, n % 4)
            colorsp_chooses.metric_choose_signal.connect(self.change_metric)
            self.colorsp_choosess.append(colorsp_chooses)
        self.device_grids.setRowStretch(nrow + 1, 1)  # 第1行（空行）拉伸占满剩余空间
        self.set_default_size_policy(self.colorsp_choosess)
    ...
    def clear(self):
        for i in range(self.choose_layout.count()):
            wid = self.choose_layout.itemAt(i).widget()
            wid.deleteLater()
        for color_choose in self.colorsp_choosess:
            color_choose.clearcheck()
        self.update()
        self.repaint()
...
class RecordForm(QTableWidget):
    ...
    def set_zoom_size(self, ratio=1):
        ...
        self.horizontalHeader().setFixedHeight(defaultHeight / (self.nrow + 1)*1.1)
        font.setBold(True)
        self.horizontalHeader().setFont(font)
    ...
    def adjust_column_width(self, refwidth=0):
        ...
        width = max(self.ncol * self.defaultWidth * self.ratio * 0.95, refwidth)
        self.setFixedWidth(width)

    def get_width_hint(self):
        self.ncol = max(len(self.metrics), 1)
        width = max(self.ncol * self.defaultWidth * self.ratio * 0.95, 0)
        return width
